...
const defaultState = {
  activeView: 'today',
  watched: { severance: 13, 'last-of-us': 10, 'white-lotus': 15, andor: 7, 'slow-horses': 24, arcane: 18, shogun: 10, dune: 0, ripley: 0, fallout: 0 },
  watchedEpisodes: {},
  lists: [
...
function renderCalendarGuide() {
  ...
  const releaseByDay = new Map(releases.filter(item => item.date.getFullYear() === year && item.date.getMonth() === month).map(item => [item.date.getDate(), item]));
...
async function openShow(id) {
  ...
  document.getElementById('modal-watch-button').addEventListener('click', () => { if (show.type === 'Movie') { state.watched[id] = 1; } else if (episodes.length) { setEpisodeWatched(show, episodes[watched] || episodes[episodes.length - 1], watched, true); } else { state.watched[id] = Math.min(show.episodes, watched + 1); saveState(); } show.status = state.watched[id] >= show.episodes ? 'Completed' : 'Watching'; saveCatalog(); recordActivity(id, `Watched ${show.next}`); closeModal(); render(); showToast(`${show.title} progress updated`); });
...
function addToWatchlist(id) { const show = getShow(id); if (!show) return; show.status = 'Watchlist'; state.watched[id] = state.watched[id] || 0; saveCatalog(); recordActivity(id, 'Added to watchlist'); showToast(`${show.title} added to your watchlist`); render(); }
function bindViewEvents() {
  ...
  document.querySelectorAll('[data-add-id]').forEach(button => button.addEventListener('click', event => { event.stopPropagation(); addToWatchlist(button.dataset.addId); }));
