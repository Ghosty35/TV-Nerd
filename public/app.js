...
const defaultState = {
  ...
  theme: 'midnight',
  notifPrefs: { releases: true, releaseLeadDays: 1, suggestions: true, friendActivity: true, partyReminders: true, weeklyDigest: false, browserPush: false },
  notifications: [],
  customSources: [
    { id: 'yt', name: 'YouTube', color: '#ff0000', home: 'https://www.youtube.com/', search: q => `https://www.youtube.com/results?search_query=${encodeURIComponent(q)}`, default: true },
    { id: 'ext', name: 'EXT', color: '#e69b68', home: 'https://ext.to/', search: q => `https://ext.to/browse/?q=${encodeURIComponent(q)}`, default: true },
    { id: 'psa', name: 'PSA', color: '#e3a5c8', home: 'https://psa.wf/', search: q => `https://psa.wf/?s=${encodeURIComponent(q)}`, default: true }
  ],
  parties: []
};
let state = loadState();
...
/* ---------- Themes ---------- */
const THEMES = [
  { id: 'midnight', name: 'Midnight', accent: '#e16d43', swatch: '#0f1315' },
  { id: 'cinema', name: 'Cinema', accent: '#e05d3f', swatch: '#1a0f0d' },
  { id: 'ocean', name: 'Ocean', accent: '#4fb3a6', swatch: '#0b1a24' },
  { id: 'forest', name: 'Forest', accent: '#7cb87e', swatch: '#101c12' },
  { id: 'ember', name: 'Ember', accent: '#e69b3c', swatch: '#1c1309' },
  { id: 'royal', name: 'Royal', accent: '#a78bfa', swatch: '#141028' }
];
function applyTheme() { document.body.dataset.theme = state.theme || 'midnight'; }
applyTheme();

/* ---------- Countdown helpers ---------- */
function nextReleaseFor(show) {
  if (!show) return null;
  const now = Date.now();
  const fromEpisodes = (show.episodeData || [])
    .filter(episode => episode.airdate)
    .map(episode => ({ episode, date: new Date(`${episode.airdate}T${episode.airtime || '18:00:00'}`).getTime() }))
    .filter(item => !Number.isNaN(item.date) && item.date >= now)
    .sort((a, b) => a.date - b.date)[0];
  if (fromEpisodes) return fromEpisodes;
  const fromGuide = releaseItems().find(item => item.show && String(item.show.id) === String(show.id));
  return fromGuide ? { episode: fromGuide.episode, date: fromGuide.date.getTime() } : null;
}
...
function countdownLabel(target) {
  const { days, hours, minutes, seconds } = countdownParts(target);
...
function tickCountdowns() {
  document.querySelectorAll('[data-countdown]').forEach(element => {
    const target = Number(element.dataset.countdown);
    if (!target) return;
    element.textContent = countdownLabel(target);
    element.classList.toggle('countdown-live', target > Date.now());
  });
}
...
function pushNotification(item) {
  ...
  const same = state.notifications.find(n => n.key === item.key);
  if (same) { same.at = Date.now(); same.read = false; return; }
  state.notifications.unshift({ ...item, key: item.key || `n-${Date.now()}`, at: Date.now(), read: false });
...
function generateNotifications() {
  ...
  if (prefs.releases !== false) {
    const leadMs = (Number(prefs.releaseLeadDays) || 1) * 86400000;
    releaseItems().slice(0, 6).forEach(item => {
      if (item.date.getTime() - Date.now() <= leadMs) {
        pushNotification({ key: `rel-${item.show?.id}-${item.date.getTime()}`, type: 'release', icon: '📺', title: `${item.show?.title || 'New episode'} airs ${item.date.toLocaleDateString('en-US', { weekday: 'short', month: 'short', day: 'numeric' })}`, body: `${item.episode?.name || `S${item.episode?.season || '?'} E${item.episode?.number || '?'}`} — ${countdownLabel(item.date.getTime())} to go.`, showId: item.show?.id, action: 'open' });
      }
    });
  }
  if (prefs.suggestions !== false) {
    const suggested = catalog.filter(show => isUserAdded(show.id) && show.status !== 'Completed').slice(0, 2);
    suggested.forEach(show => pushNotification({ key: `sug-${show.id}`, type: 'suggestion', icon: '💡', title: `Keep watching ${show.title}`, body: `${show.next || 'Your next episode'} is waiting in your library.`, showId: show.id, action: 'open' }));
  }
  if (prefs.friendActivity !== false) {
    currentProfile().friends.slice(0, 2).forEach(friend => pushNotification({ key: `fr-${friend.id}`, type: 'friend', icon: '👥', title: `${friend.name} is watching`, body: friend.status || 'Check out their feed.', action: 'party' }));
  }
  if (prefs.partyReminders !== false) {
    state.parties.forEach(party => {
      const diff = party.when - Date.now();
      if (diff > 0 && diff < 3 * 86400000) pushNotification({ key: `party-${party.id}`, type: 'party', icon: '🎉', title: 'Watch party soon', body: `${getShow(party.showId)?.title || 'Your party'} starts ${countdownLabel(party.when)} from now.`, showId: party.showId, action: 'party' });
    });
  }
}
function updateBell() {
  const badge = document.getElementById('notif-badge');
  const dot = document.querySelector('#notifications-button i');
...
function renderNotifPanel() {
  ...
  const unreadLabel = document.getElementById('notif-unread');
  const unread = state.notifications.filter(n => !n.read).length;
  if (unreadLabel) unreadLabel.textContent = unread ? `${unread} new` : 'You are all caught up';
  if (!list) return;
  list.innerHTML = state.notifications.length ? state.notifications.map(n => `<button class="notif-item ${n.read ? '' : 'unread'}" data-notif-action="${n.action || 'open'}" data-notif-show="${n.showId || ''}" type="button"><span class="notif-icon">${n.icon || '🔔'}</span><span class="notif-copy"><strong>${escapeHtml(n.title)}</strong><small>${escapeHtml(n.body || '')}</small><time>${timeAgo(n.at)}</time></span></button>`).join('') : '<div class="empty-state notif-empty">No notifications yet. Release alerts, suggestions and party reminders land here.</div>';
}
...
function sourceLinksFor(show) { const query = show.type === 'Movie' ? `${show.title} ${show.year} 1080p` : `${show.title} ${show.next || ''}`.replace(/\s+/g, ' ').trim(); return state.customSources.filter(s => s.id !== 'yt').map(source => { let href; try { href = typeof source.search === 'function' ? source.search(query) : String(source.search || '').replace('{q}', encodeURIComponent(query)); } catch { href = source.home || '#'; } return `<a class="watch-link torrent-watch-link" href="${escapeHtml(href)}" target="_blank" rel="noopener noreferrer" data-external-link="${escapeHtml(source.id)}" title="Search ${escapeHtml(show.title)} on ${escapeHtml(source.name)}"><span class="watch-icon" aria-hidden="true">⇣</span><span>${escapeHtml(source.name)}</span><span class="external-mark" aria-hidden="true">↗</span></a>`; }).join(''); }
...
function upcomingCountdownBanner() {
  const next = releaseItems()[0];
  if (!next || !next.date) return '';
  const diff = next.date.getTime() - Date.now();
  if (diff <= 0) return '';
  return `<div class="upcoming-banner"><span class="upcoming-banner-icon">⏳</span><div><strong>${escapeHtml(next.show?.title || 'Next release')}</strong><span>${escapeHtml(next.episode?.name || `S${next.episode?.season || 1} E${next.episode?.number || 1}`)} · <b data-countdown="${next.date.getTime()}">${escapeHtml(countdownLabel(next.date.getTime()))}</b> to go</span></div><button class="button glass-button" data-show-id="${escapeHtml(next.show?.id || '')}" type="button">View</button></div>`;
}
function nextReleaseBanner(show) {
  const next = nextReleaseFor(show);
  if (!next) return '';
  const diff = next.date - Date.now();
  if (diff <= 0) return '';
  return `<div class="modal-countdown"><span>⏳</span><div><strong>Next ${show.type === 'Movie' ? 'release' : 'episode'}: ${escapeHtml(next.episode?.name || 'TBA')}</strong><span><b data-countdown="${next.date}">${escapeHtml(countdownLabel(next.date))}</b> to go · ${new Date(next.date).toLocaleString('en-US', { weekday: 'long', month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' })}</span></div></div>`;
}
...
function triviaFor(show) {
  const base = [
    `${show.title} (${show.year}) is a ${show.type} in the ${show.genre.split('/')[0].trim()} lane on your shelf.`,
    `Your progress: ${watchedEpisodesFor(show)} of ${show.episodes || '—'} episodes watched (${progressFor(show)}%).`,
    `Rated ${show.rating || 'N/A'}/10 by the community — ${show.rating >= 8 ? 'critically adored.' : show.rating >= 7 ? 'solidly above average.' : 'a hidden gem waiting for its audience.'}`,
    show.runtime ? `Typical runtime: ${show.runtime} per episode — ${show.type === 'Movie' ? 'a feature-length commitment.' : `a ${(parseInt(show.runtime) / 60 || 0.75).toFixed(1)}-hour block if you binge.`}` : 'Runtime info is still loading for this title.'
  ];
  if (show.type === 'Movie') base.push('Movies count as one episode in your watch history.');
  else if (show.episodes) base.push(`Finish the whole run and you are looking at roughly ${Math.round((parseInt(show.runtime) || 45) * show.episodes / 60)} hours of story.`);
  return base;
}
function shareCardMarkup(show) {
  const initials = (show.title || 'TV').split(/\s+/).map(w => w[0]).join('').slice(0, 2).toUpperCase();
  return `<div class="watch-card"><div class="watch-card-art" style="background-image:url('${escapeHtml(show.backdrop || show.poster)}')"><div class="watch-card-shade"></div><div class="watch-card-top"><img class="brand-mark-icon" src="/icon.svg" alt=""><span>TVNerd</span></div><div class="watch-card-bottom"><div class="watch-card-poster-wrap">${poster(show, 'watch-card-poster')}</div><div><span class="eyebrow">${show.type} · ${escapeHtml(show.year)}</span><h3>${escapeHtml(show.title)}</h3><div class="watch-card-meta"><span>★ ${show.rating || 'N/A'}</span><span>${escapeHtml(show.genre)}</span><span>${show.runtime}</span></div><p>${escapeHtml((show.description || '').slice(0, 140))}${(show.description || '').length > 140 ? '…' : ''}</p><span class="watch-card-footer">${initials} is watching on TVNerd · ${progressFor(show)}% through</span></div></div></div></div>`;
}
...
function renderCalendarGuide() {
  ...
  const episodeLabel = item => item.episode ? (item.episode.season ? `S${item.episode.season} E${item.episode.number}` : (item.episode.name || item.detail || 'New release')) : (item.detail || 'New release');
  const releaseCard = item => { const show = item.show; return `<button class="calendar-release-card" data-show-id="${escapeHtml(show.id)}" type="button" title="Open ${escapeHtml(show.title)} details">${poster(show, 'calendar-release-poster')}<span class="calendar-release-copy"><strong>${escapeHtml(show.title)}</strong><small>${escapeHtml(episodeLabel(item))}</small></span></button>`; };
  const headerLabel = view === 'month' ? cursor.toLocaleString('en-US', { month: 'long', year: 'numeric' }) : view === 'week' ? `Week of ${new Date(cursor.getFullYear(), cursor.getMonth(), cursor.getDate() - ((cursor.getDay() + 6) % 7)).toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}` : cursor.toLocaleDateString('en-US', { weekday: 'long', month: 'long', day: 'numeric', year: 'numeric' });
  let body = '';
  if (view === 'month') {
    ...
    body = `<div class="large-calendar-grid">${['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'].map(d => `<div class="weekday">${d}</div>`).join('')}${cells.map(day => { const items = releaseByDay.get(day) || []; const isToday = day === today.getDate() && month === today.getMonth() && year === today.getFullYear(); const cards = items.slice(0, 3).map(releaseCard).join(''); const more = items.length > 3 ? `<span class="calendar-more">+${items.length - 3}</span>` : ''; return `<div class="calendar-cell ${day < 1 || day > daysInMonth ? 'calendar-outside' : ''}"><span class="calendar-number ${isToday ? 'today-number' : ''}">${day < 1 ? new Date(year, month, day).getDate() : day > daysInMonth ? day - daysInMonth : day}</span>${cards}${more}</div>`; }).join('')}</div>`;
  } else if (view === 'week') {
    const monday = new Date(year, month, cursor.getDate() - ((cursor.getDay() + 6) % 7));
    const weekDays = Array.from({ length: 7 }, (_, i) => new Date(monday.getFullYear(), monday.getMonth(), monday.getDate() + i));
    body = `<div class="week-calendar"><div class="week-grid">${weekDays.map(day => { const items = releases.filter(item => sameDay(item.date, day)); const isToday = sameDay(day, today); const d = day.getDate(); const cards = items.map(releaseCard).join(''); return `<div class="week-cell ${isToday ? 'week-today' : ''}"><div class="week-cell-head"><span>${day.toLocaleDateString('en-US', { weekday: 'short' })}</span><b class="${isToday ? 'today-number' : ''}">${d}</b></div><div class="week-cell-body">${cards || '<span class="week-empty">No releases</span>'}</div></div>`; }).join('')}</div></div>`;
  } else {
    const dayItems = releases.filter(item => sameDay(item.date, cursor)).sort((a, b) => a.date - b.date);
    body = `<div class="day-calendar"><div class="day-calendar-head"><span class="eyebrow">${cursor.toLocaleDateString('en-US', { weekday: 'long' })} · ${sameDay(cursor, today) ? 'TODAY' : ''}</span><h2>${cursor.toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' })}</h2></div><div class="day-release-list">${dayItems.length ? dayItems.map(item => `<button class="day-release-item" data-show-id="${item.show.id}" type="button">${poster(item.show, 'day-release-poster')}<span class="day-release-copy"><strong>${escapeHtml(item.show.title)}</strong><small>${escapeHtml(episodeLabel(item))}</small><em>${item.show.type} · ${item.show.runtime || 'Runtime TBA'}</em></span><span class="day-release-time">${item.date.toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit' })}</span><span class="agenda-arrow">›</span></button>`).join('') : '<div class="empty-state">Nothing releases on this day.</div>'}</div></div>`;
  }
  appView.innerHTML = `<div class="page-head"><div><div class="eyebrow">TV guide and premieres</div><h1 class="page-title">Release guide</h1><p class="page-subtitle">Track premieres, new episodes, movies, and the titles you marked as favorites.</p></div><div class="head-actions"><button class="button ${mode === 'calendar' ? 'primary' : ''}" id="calendar-mode-button" type="button">Calendar</button><button class="button ${mode === 'agenda' ? 'primary' : ''}" id="agenda-mode-button" type="button">Agenda</button><button class="button" id="export-calendar-button" type="button">Export .ics</button>${releases[0] ? `<button class="button google-guide-button" id="google-next-button" type="button">Add next to Google</button>` : ''}</div></div><div class="guide-toolbar"><div class="guide-filters" role="group" aria-label="Release filters"><button class="guide-filter ${state.calendarFilter === 'all' ? 'active' : ''}" data-calendar-filter="all" type="button">All releases</button><button class="guide-filter ${state.calendarFilter === 'Series' ? 'active' : ''}" data-calendar-filter="Series" type="button">Series</button><button class="guide-filter ${state.calendarFilter === 'Movie' ? 'active' : ''}" data-calendar-filter="Movie" type="button">Movies</button><button class="guide-filter ${state.calendarFilter === 'favorites' ? 'active' : ''}" data-calendar-filter="favorites" type="button">★ Favorites</button></div><span class="source-label">${releases.length} upcoming</span></div>${mode === 'agenda' ? renderAgenda(releases) : `<section class="calendar-page"><div class="calendar-toolbar"><div class="calendar-nav"><button class="calendar-nav-btn" id="calendar-prev" type="button" aria-label="Previous">←</button><button class="calendar-nav-btn" id="calendar-today" type="button">Today</button><button class="calendar-nav-btn" id="calendar-next" type="button" aria-label="Next">→</button></div><div class="calendar-view-switch"><button class="calendar-view-btn ${view === 'day' ? 'active' : ''}" data-calendar-view="day" type="button">Day</button><button class="calendar-view-btn ${view === 'week' ? 'active' : ''}" data-calendar-view="week" type="button">Week</button><button class="calendar-view-btn ${view === 'month' ? 'active' : ''}" data-calendar-view="month" type="button">Month</button></div></div><div class="large-calendar"><div class="large-calendar-header"><div><h2 class="section-title">${headerLabel}</h2><span class="section-meta">${releases.length} upcoming releases found</span></div></div>${body}</div><div class="panel release-list-panel"><div class="panel-header"><h2 class="panel-title">Upcoming releases</h2><span class="section-meta">Next ${Math.min(releases.length, 8)}</span></div><div class="upcoming-mini">${releases.slice(0, 8).map(miniUpcoming).join('') || '<div class="empty-state">No upcoming dates available yet.</div>'}</div></div></section>`}`;
  ...
  document.querySelectorAll('[data-calendar-view]').forEach(button => button.addEventListener('click', () => { state.calendarView = button.dataset.calendarView; saveState(); renderCalendarGuide(); }));
  document.getElementById('calendar-prev')?.addEventListener('click', () => { const d = new Date(cursor); view === 'day' ? d.setDate(d.getDate() - 1) : view === 'week' ? d.setDate(d.getDate() - 7) : d.setMonth(d.getMonth() - 1); state.calendarCursor = d.toISOString(); saveState(); renderCalendarGuide(); });
  document.getElementById('calendar-next')?.addEventListener('click', () => { const d = new Date(cursor); view === 'day' ? d.setDate(d.getDate() + 1) : view === 'week' ? d.setDate(d.getDate() + 7) : d.setMonth(d.getMonth() + 1); state.calendarCursor = d.toISOString(); saveState(); renderCalendarGuide(); });
  document.getElementById('calendar-today')?.addEventListener('click', () => { state.calendarCursor = new Date().toISOString(); saveState(); renderCalendarGuide(); });
...
function localSearchMarkup(results) { return results.length ? results.map(show => `<div class="search-result clickable-result" data-show-id="${show.id}" title="Open ${escapeHtml(show.title)}">${poster(show)}<div class="search-result-copy"><strong>${escapeHtml(show.title)}</strong><span>${show.year} · ${show.type} · ${escapeHtml(show.genre)}</span><span>★ ${show.rating || 'N/A'}</span></div><div class="search-result-actions">${imdbLink(show, true)}<button class="button ${isUserAdded(show.id) ? 'primary' : ''}" data-add-id="${show.id}" type="button">${isUserAdded(show.id) ? '✓ In your library' : '+ Add to library'}</button></div></div>`).join('') : '<div class="empty-state">No local titles found. Search the live catalog below.</div>'; }
...
async function searchRemote(query) {
  ...
  try {
    ...
    container.querySelectorAll('[data-import-remote]').forEach(button => button.addEventListener('click', event => { event.stopPropagation(); importRemoteShow(results.find(item => String(item.show.id) === button.dataset.importRemote)?.show); }));
    container.querySelectorAll('.search-result[data-remote-id]').forEach(card => card.addEventListener('click', async () => { const remote = results.find(item => String(item.show.id) === card.dataset.remoteId)?.show; if (!remote) return; await importRemoteShow(remote); const local = getShow(`tvmaze-${remote.id}`); if (local) openShow(local.id); else showToast('Opening this title needs an internet connection'); }));
  } catch {
...
async function openShow(id) {
  ...
  modal.innerHTML = `<div class="cinema-detail" style="--show-backdrop: url('${escapeHtml(backdrop)}')"><div class="cinema-detail-shade"></div><header class="cinema-detail-top"><span class="eyebrow">SHOW DETAILS</span><button class="modal-close" id="close-modal" type="button" aria-label="Close">×</button></header><div class="cinema-hero-copy"><div class="cinema-poster-wrap">${poster(show, 'cinema-poster')}</div><div class="cinema-title-stack"><div class="eyebrow">${show.type} · ${escapeHtml(show.year)} · ${escapeHtml(show.genre)}</div><h2 id="modal-title">${escapeHtml(show.title)}</h2><div class="cinema-meta"><span>★ ${show.rating}/10</span><span>${show.episodes} ${show.type === 'Movie' ? 'film' : 'episodes'}</span><span>${escapeHtml(show.runtime || '45 min')}</span></div>${nextReleaseBanner(show)}<p class="modal-copy">${escapeHtml(show.description)}</p><div class="modal-actions"><button class="button primary" id="modal-watch-button" type="button">${show.status === 'Completed' ? 'Rewatch' : `Mark ${show.next} watched`}</button><button class="button" id="modal-reminder-button" type="button">${state.reminders ? 'Reminder set' : 'Set reminder'}</button>${favoriteButton(show)}<button class="button share-button" id="modal-share-button" type="button" title="Share this title">↗ Share</button><button class="button" id="modal-card-button" type="button" title="Make a share card">🖼 Card</button>${isUserAdded(id) ? '<button class="button danger-button" id="modal-remove-button" type="button" title="Remove from your library">− Remove from library</button>' : ''}${watchLink(show)}${sourceLinksFor(show)}${trailerLink(show)}${imdbLink(show)}</div></div></div><div class="cinema-content-dock">${trailerPanelMarkup()}<div class="detail-tabs" role="tablist"><button class="detail-tab active" data-modal-tab="overview" role="tab" type="button">Overview</button>${show.type !== 'Movie' ? '<button class="detail-tab" data-modal-tab="episodes" role="tab" type="button">Episodes</button>' : ''}<button class="detail-tab" data-modal-tab="trivia" role="tab" type="button">Trivia</button><button class="detail-tab" data-modal-tab="notes" role="tab" type="button">My details</button></div><section class="detail-panel active" data-modal-panel="overview"><div class="show-controls"><label><span>Library status</span><select id="show-status-select"><option value="Watchlist" ${show.status === 'Watchlist' ? 'selected' : ''}>Watchlist</option><option value="Up next" ${show.status === 'Up next' ? 'selected' : ''}>Up next</option><option value="Watching" ${show.status === 'Watching' ? 'selected' : ''}>Watching</option><option value="Completed" ${show.status === 'Completed' ? 'selected' : ''}>Completed</option></select></label><div><span class="control-label">Your rating</span><div class="star-picker" role="group" aria-label="Your rating">${[1, 2, 3, 4, 5].map(rating => `<button class="rating-star ${Number(state.ratings[id]) >= rating ? 'selected' : ''}" data-rating="${rating}" title="Rate ${rating} out of 5" type="button">★</button>`).join('')}</div></div></div><div class="list-assignment"><label for="show-list-select">Add to a list</label><select id="show-list-select"><option value="">Choose a list</option>${state.lists.map(list => `<option value="${escapeHtml(list.id)}">${escapeHtml(list.name)}</option>`).join('')}</select></div></section>${show.type !== 'Movie' ? `<section class="detail-panel" data-modal-panel="episodes"><div class="episode-tab-heading"><div><span class="eyebrow">WATCH ORDER</span><h3 class="section-title">Episode progress</h3></div><span class="section-meta">${watched}/${show.episodes} watched</span></div><div class="episode-checklist">${episodeItems}</div></section>` : ''}<section class="detail-panel" data-modal-panel="trivia"><div class="episode-tab-heading"><div><span class="eyebrow">DID YOU KNOW?</span><h3 class="section-title">Fun facts</h3></div></div><div class="trivia-list">${triviaFor(show).map(fact => `<div class="trivia-item"><span>💡</span><p>${escapeHtml(fact)}</p></div>`).join('')}</div></section><section class="detail-panel" data-modal-panel="notes"><div class="episode-tab-heading"><div><span class="eyebrow">PRIVATE SPACE</span><h3 class="section-title">Your note</h3></div></div><textarea id="show-note" placeholder="What do you think so far?">${escapeHtml(state.notes[id] || '')}</textarea><button class="button" id="save-note" type="button">Save note</button></section></div></div>`;
  ...
  document.getElementById('modal-card-button')?.addEventListener('click', () => { const overlay = document.createElement('div'); overlay.className = 'card-overlay'; overlay.innerHTML = `<div class="card-modal"><button class="card-close" type="button" aria-label="Close">×</button>${shareCardMarkup(show)}<div class="card-modal-actions"><button class="button primary" id="card-download" type="button">⬇ Download image</button><button class="button" id="card-share" type="button">↗ Share text</button></div></div>`; document.body.appendChild(overlay); overlay.querySelector('.card-close').addEventListener('click', () => overlay.remove()); overlay.addEventListener('click', event => { if (event.target === overlay) overlay.remove(); }); overlay.querySelector('#card-download').addEventListener('click', () => { const node = overlay.querySelector('.watch-card'); import('https://cdn.jsdelivr.net/npm/html-to-image@1.11.11/+esm').then(async ({ toPng }) => { try { const dataUrl = await toPng(node, { pixelRatio: 2, backgroundColor: '#0f1315' }); const link = document.createElement('a'); link.download = `${show.title.replace(/[^a-z0-9]+/gi, '-').toLowerCase()}-tvnerd.png`; link.href = dataUrl; link.click(); } catch { showToast('Image export needs an internet connection'); } }).catch(() => showToast('Image export is unavailable offline')); }); overlay.querySelector('#card-share').addEventListener('click', () => shareText(`Check out ${show.title} (${show.year}) — ★ ${show.rating}/10 on TVNerd. My watch progress: ${progressFor(show)}%.`)); });
...
function bindLibraryFilters() {
  const search = document.getElementById('library-search'); const status = document.getElementById('library-status-filter'); const type = document.getElementById('library-type-filter'); const decade = document.getElementById('library-decade-filter'); const rating = document.getElementById('library-rating-filter');
  const apply = () => { const query = search.value.toLowerCase(); document.querySelectorAll('#library-grid .library-card').forEach(card => { const show = getShow(card.dataset.showId); const decadeMatch = !decade || decade.value === 'all' || (show.year && `${String(show.year).slice(0, 3)}0s` === decade.value); const ratingMatch = !rating || rating.value === 'all' || Number(show.rating) >= Number(rating.value); const matches = (!query || `${show.title} ${show.genre}`.toLowerCase().includes(query)) && (status.value === 'all' || show.status === status.value) && (type.value === 'all' || show.type === type.value) && decadeMatch && ratingMatch; card.style.display = matches ? '' : 'none'; }); };
  search?.addEventListener('input', apply); status?.addEventListener('change', apply); type?.addEventListener('change', apply); decade?.addEventListener('change', apply); rating?.addEventListener('change', apply);
...
function render() { updateChrome(); document.body.classList.toggle('stream-home-active', state.activeView === 'today'); if (state.activeView?.startsWith('list:')) renderList(state.activeView.slice(5)); else if (state.activeView === 'shows') renderShows(); else if (state.activeView === 'calendar') renderCalendar(); else if (state.activeView === 'discover') renderDiscover(); else if (state.activeView === 'search') renderSearch(); else if (state.activeView === 'profile') renderProfile(); else if (state.activeView === 'legal') renderLegal(); else if (state.activeView === 'stats') renderStats(); else if (state.activeView === 'randomizer') renderRandomizer(); else if (state.activeView === 'party') renderParty(); else if (state.activeView === 'guide') renderGuide(); else renderToday(); }

// ---------- Badges ----------
const BADGES = [
  { id: 'first-add', icon: '🌱', name: 'First Steps', desc: 'Add your first title to the library', check: s => s.userAddedIds.length >= 1 },
  { id: 'lib-5', icon: '📚', name: 'Library Builder', desc: 'Add 5 titles to your library', check: s => s.userAddedIds.length >= 5 },
  { id: 'lib-15', icon: '🏛', name: 'Collector', desc: 'Add 15 titles to your library', check: s => s.userAddedIds.length >= 15 },
  { id: 'complete-1', icon: '🏁', name: 'Finisher', desc: 'Complete one title', check: s => catalog.filter(sh => isUserAdded(sh.id) && sh.status === 'Completed').length >= 1 },
  { id: 'complete-5', icon: '🎖', name: 'Marathoner', desc: 'Complete 5 titles', check: s => catalog.filter(sh => isUserAdded(sh.id) && sh.status === 'Completed').length >= 5 },
  { id: 'watched-20', icon: '▶️', name: 'Binge Beginner', desc: 'Watch 20 episodes', check: s => watchedCount() >= 20 },
  { id: 'watched-100', icon: '🔥', name: 'Binge Machine', desc: 'Watch 100 episodes', check: s => watchedCount() >= 100 },
  { id: 'favorite-1', icon: '⭐', name: 'Fan Favorite', desc: 'Favorite your first title', check: s => currentProfile().favoriteIds.length >= 1 },
  { id: 'party-1', icon: '🎉', name: 'Host', desc: 'Plan your first watch party', check: s => s.parties.length >= 1 },
  { id: 'rated-3', icon: '🗣', name: 'Critic', desc: 'Rate 3 titles', check: s => Object.keys(s.ratings).length >= 3 },
  { id: 'note-1', icon: '📝', name: 'Scribbler', desc: 'Save a personal note', check: s => Object.keys(s.notes).length >= 1 },
  { id: 'list-1', icon: '🗂', name: 'Organizer', desc: 'Create a custom list', check: s => (s.lists || []).length >= 1 }
];
function earnedBadges() { return BADGES.filter(b => b.check(state)); }
function badgeMarkup() { const earned = earnedBadges(); return `<div class="badge-grid">${BADGES.map(b => { const has = earned.includes(b); return `<div class="badge ${has ? 'earned' : 'locked'}" title="${escapeHtml(b.desc)}"><span class="badge-icon">${b.icon}</span><strong>${escapeHtml(b.name)}</strong><small>${escapeHtml(b.desc)}</small>${has ? '<em>EARNED</em>' : '<em>LOCKED</em>'}</div>`; }).join('')}</div>`; }

// ---------- Stats ----------
function renderStats() {
  const library = catalog.filter(show => isUserAdded(show.id));
  const episodesWatched = watchedCount();
  const minutesWatched = library.reduce((sum, show) => sum + (Number(state.watched[show.id] || 0) * (parseInt(show.runtime) || 45)), 0);
  const completed = library.filter(show => show.status === 'Completed').length;
  const genres = {};
  library.forEach(show => (show.genre || 'Other').split('/').map(g => g.trim()).forEach(g => { genres[g] = (genres[g] || 0) + 1; }));
  const genreRows = Object.entries(genres).sort((a, b) => b[1] - a[1]).slice(0, 8);
  const maxGenre = genreRows.length ? genreRows[0][1] : 1;
  const last30 = {};
  for (let i = 29; i >= 0; i--) { const d = new Date(); d.setDate(d.getDate() - i); last30[d.toISOString().slice(0, 10)] = 0; }
  state.activity.forEach(a => { const day = a.at.slice(0, 10); if (day in last30) last30[day] += 1; });
  const maxDay = Math.max(1, ...Object.values(last30));
  const avgRating = library.length ? (Object.entries(state.ratings).filter(([id]) => getShow(id) && isUserAdded(id)).reduce((s, [, r]) => s + r, 0) / Math.max(1, Object.entries(state.ratings).filter(([id]) => getShow(id) && isUserAdded(id)).length)).toFixed(1) : '—';
  const hours = Math.floor(minutesWatched / 60); const mins = minutesWatched % 60;
  appView.innerHTML = `<div class="page-head"><div><div class="eyebrow">Your watch analytics</div><h1 class="page-title">Stats &amp; insights</h1><p class="page-subtitle">Everything you have watched, ranked, and collected — at a glance.</p></div><div class="head-actions"><button class="button" data-nav="shows" type="button">Open library</button></div></div><div class="stats-dashboard"><section class="stats-hero-grid">${[['⏱', `${hours}h ${mins}m`, 'Total watch time'], ['▶', episodesWatched, 'Episodes watched'], ['🏁', completed, 'Titles completed'], ['⭐', avgRating, 'Average rating']].map(([icon, value, label]) => `<div class="stat-hero-card"><span class="stat-hero-icon">${icon}</span><strong>${value}</strong><small>${label}</small></div>`).join('')}</section><div class="stats-columns"><section class="panel stats-panel"><div class="panel-header"><div><h2 class="panel-title">Genre breakdown</h2><span class="section-meta">What your library is made of</span></div></div>${genreRows.length ? `<div class="genre-bars">${genreRows.map(([genre, count]) => `<div class="genre-bar-row"><span>${escapeHtml(genre)}</span><div class="genre-bar-track"><i style="width:${Math.round(count / maxGenre * 100)}%"></i></div><b>${count}</b></div>`).join('')}</div>` : '<div class="empty-state">Add titles to see your genre mix.</div>'}</section><section class="panel stats-panel"><div class="panel-header"><div><h2 class="panel-title">Last 30 days</h2><span class="section-meta">Watch activity</span></div></div>${Object.values(last30).some(v => v > 0) ? `<div class="activity-chart">${Object.entries(last30).map(([day, count]) => `<div class="chart-day" title="${day}: ${count} action${count === 1 ? '' : 's'}"><i style="height:${Math.max(8, Math.round(count / maxDay * 100))}%"></i></div>`).join('')}</div><div class="chart-axis"><span>30 days ago</span><span>Today</span></div>` : '<div class="empty-state">Watch, rate, or add titles to see your activity chart.</div>'}</section><section class="panel stats-panel"><div class="panel-header"><div><h2 class="panel-title">Achievements</h2><span class="section-meta">${earnedBadges().length} of ${BADGES.length} unlocked</span></div></div>${badgeMarkup()}</section></div></div>`;
  bindViewEvents();
}
...
function renderRandomizer() {
  const library = catalog.filter(show => isUserAdded(show.id));
  const genres = [...new Set(library.flatMap(show => (show.genre || '').split('/').map(g => g.trim())))];
  const decades = [...new Set(library.map(show => show.year ? `${String(show.year).slice(0, 3)}0s` : 'Unknown'))];
  appView.innerHTML = `<div class="page-head"><div><div class="eyebrow">Decision maker</div><h1 class="page-title">Randomizer</h1><p class="page-subtitle">Can't decide? Spin the wheel — TVNerd picks from your library.</p></div></div><div class="randomizer-panel panel"><div class="randomizer-filters"><label>Type<select id="random-type"><option value="all">Any</option><option value="Series">Series</option><option value="Movie">Movie</option></select></label><label>Genre<select id="random-genre"><option value="all">Any genre</option>${genres.map(g => `<option value="${escapeHtml(g)}">${escapeHtml(g)}</option>`).join('')}</select></label><label>Decade<select id="random-decade"><option value="all">Any decade</option>${decades.map(d => `<option value="${escapeHtml(d)}">${escapeHtml(d)}</option>`).join('')}</select></label><label>Status<select id="random-status"><option value="all">Any status</option><option value="Watching">Watching</option><option value="Watchlist">Watchlist</option><option value="Completed">Completed</option></select></label></div><div class="randomizer-stage"><div class="randomizer-reel" id="randomizer-reel">${library.length ? 'Press spin to pick something to watch.' : 'Your library is empty — add titles first.'}</div><button class="button primary randomizer-spin" id="randomizer-spin" type="button" ${library.length ? '' : 'disabled'}>🎲 Spin the wheel</button><div class="randomizer-result" id="randomizer-result" hidden></div></div></div>`;
  bindViewEvents();
  document.getElementById('randomizer-spin')?.addEventListener('click', () => { const type = document.getElementById('random-type').value; const genre = document.getElementById('random-genre').value; const decade = document.getElementById('random-decade').value; const status = document.getElementById('random-status').value; let pool = library.filter(show => (type === 'all' || show.type === type) && (genre === 'all' || (show.genre || '').toLowerCase().includes(genre.toLowerCase())) && (decade === 'all' || (show.year ? `${String(show.year).slice(0, 3)}0s` : 'Unknown') === decade) && (status === 'all' || show.status === status)); if (!pool.length) { document.getElementById('randomizer-result').hidden = false; document.getElementById('randomizer-result').innerHTML = '<div class="empty-state">No titles match those filters.</div>'; return; } const reel = document.getElementById('randomizer-reel'); const result = document.getElementById('randomizer-result'); result.hidden = true; reel.textContent = ''; let spins = 0; const total = 12 + Math.floor(Math.random() * 8); const pick = () => { const show = pool[Math.floor(Math.random() * pool.length)]; reel.innerHTML = `<img class="random-reel-poster" src="${escapeHtml(show.poster)}" alt=""><strong>${escapeHtml(show.title)}</strong>`; spins++; if (spins < total) setTimeout(pick, 90); else { result.hidden = false; result.innerHTML = `<div class="random-winner"><div class="random-winner-art">${poster(show, 'random-winner-poster')}<span class="cover-badge">${show.type}</span></div><div class="random-winner-copy"><span class="eyebrow">🎲 THE WHEEL SAYS</span><h2>${escapeHtml(show.title)}</h2><p>${escapeHtml(show.description || '')}</p><div class="random-winner-meta"><span>${escapeHtml(show.year)}</span><span>${escapeHtml(show.genre)}</span><span>★ ${show.rating || 'N/A'}</span><span>${show.runtime}</span></div><div class="head-actions"><button class="button primary" data-show-id="${escapeHtml(show.id)}" type="button">Open title</button><button class="button" id="random-spin-again" type="button">Spin again</button></div></div></div>`; document.getElementById('random-spin-again').addEventListener('click', () => { result.hidden = true; document.getElementById('randomizer-spin').click(); }); const openBtn = result.querySelector('[data-show-id]'); openBtn.addEventListener('click', () => openShow(show.id)); } }; pick(); });
}
...
function renderParty() {
  const library = catalog.filter(show => isUserAdded(show.id));
  const now = Date.now();
  const sorted = [...state.parties].sort((a, b) => a.when - b.when);
  appView.innerHTML = `<div class="page-head"><div><div class="eyebrow">Watch together</div><h1 class="page-title">Watch party</h1><p class="page-subtitle">Pick a title, set a date, and get your friends on the couch.</p></div></div><div class="party-layout"><section class="panel party-form-panel"><div class="panel-header"><div><h2 class="panel-title">Plan a watch party</h2><span class="section-meta">Invites go to your friends</span></div></div><div class="party-form"><label>Title<select id="party-title"><option value="">Choose from your library…</option>${library.map(show => `<option value="${show.id}">${escapeHtml(show.title)}</option>`).join('')}</select></label><label>Date<input id="party-date" type="date" min="${new Date().toISOString().slice(0, 10)}"></label><label>Time<input id="party-time" type="time" value="20:00"></label><label>Guests<select id="party-guests" multiple size="2">${currentProfile().friends.map(f => `<option value="${escapeHtml(f.id)}">${escapeHtml(f.name)}</option>`).join('')}</select></label><button class="button primary" id="party-create" type="button">🎉 Create party</button></div></section><section class="party-list-panel"><div class="section-header"><h2 class="section-title">Your parties <span class="section-meta">${sorted.length}</span></h2></div>${sorted.length ? `<div class="party-list">${sorted.map(party => { const show = getShow(party.showId); const diff = party.when - now; const label = diff > 0 ? `${Math.ceil(diff / 86400000)}d ${Math.ceil((diff % 86400000) / 3600000)}h to go` : (Math.abs(diff) < 3600000 ? 'Happening now' : 'Happened'); return `<div class="party-card"><div class="party-card-art">${poster(show, 'party-poster')}</div><div class="party-card-copy"><strong>${escapeHtml(show?.title || 'A title')}</strong><span>${new Date(party.when).toLocaleString('en-US', { weekday: 'short', month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' })}</span><small>${party.guests.length ? `${party.guests.length} friend${party.guests.length > 1 ? 's' : ''} invited` : 'Solo watch'}</small><em class="party-countdown">${escapeHtml(label)}</em></div><button class="icon-action party-delete" data-party-id="${escapeHtml(party.id)}" type="button" aria-label="Delete party">×</button></div>`; }).join('')}</div>` : '<div class="empty-state">No parties yet — plan your first one!</div>'}</section><section class="panel friends-feed-panel"><div class="panel-header"><div><h2 class="panel-title">Friends feed</h2><span class="section-meta">What your circle is up to</span></div></div><div class="friend-feed">${currentProfile().friends.map(friend => `<div class="feed-item"><span class="friend-avatar">${escapeHtml(friend.initials)}</span><span class="feed-copy"><strong>${escapeHtml(friend.name)}</strong><small>${escapeHtml(friend.status)}</small></span><time>now</time></div>`).join('') || '<div class="empty-state">Add friends in your profile to see their activity.</div>'}</div></section></div>`;
  bindViewEvents();
  document.getElementById('party-create')?.addEventListener('click', () => { const showId = document.getElementById('party-title').value; const date = document.getElementById('party-date').value; const time = document.getElementById('party-time').value || '20:00'; if (!showId || !date) return showToast('Pick a title and a date'); const when = new Date(`${date}T${time}`).getTime(); state.parties.push({ id: `party-${Date.now()}`, showId, when, guests: [...document.getElementById('party-guests').selectedOptions].map(o => o.value) }); saveState(); renderParty(); showToast('Watch party created — invite your friends!'); });
  document.querySelectorAll('.party-delete').forEach(button => button.addEventListener('click', () => { state.parties = state.parties.filter(p => p.id !== button.dataset.partyId); saveState(); renderParty(); showToast('Party removed'); }));
}

// ---------- Live guide ----------
function renderGuide() {
  const items = releaseItems().filter(item => item.show && item.date);
  const hours = []; for (let h = 8; h <= 24; h++) hours.push(h === 24 ? 0 : h);
  const slotFor = item => { const h = item.date.getHours(); return h >= 8 ? h : h + 24; };
  const byHour = {};
  items.forEach(item => { const slot = slotFor(item); (byHour[slot] = byHour[slot] || []).push(item); });
  const nowHour = new Date().getHours();
  appView.innerHTML = `<div class="page-head"><div><div class="eyebrow">Now playing</div><h1 class="page-title">Live guide</h1><p class="page-subtitle">A broadcast-style schedule of your tracked titles, hour by hour.</p></div><div class="head-actions"><button class="button" data-nav="calendar" type="button">Full calendar →</button></div></div><div class="live-guide"><div class="guide-time-axis">${hours.map(h => `<span>${String(h % 24).padStart(2, '0')}:00</span>`).join('')}</div><div class="guide-grid">${hours.map(h => `<div class="guide-hour ${h === nowHour ? 'guide-now' : ''}"><span class="guide-hour-label">${String(h % 24).padStart(2, '0')}:00</span><div class="guide-hour-cell">${(byHour[h] || []).map(item => `<button class="guide-show-chip" data-show-id="${item.show.id}" type="button">${poster(item.show, 'guide-chip-poster')}<span><strong>${escapeHtml(item.show.title)}</strong><small>${escapeHtml(item.episode?.name || `S${item.episode?.season || 1} E${item.episode?.number || 1}`)}</small></span></button>`).join('')}</div></div>`).join('')}</div></div>`;
  bindViewEvents();
}
...
function renderProfile() {
  ...
  appView.innerHTML = `<div class="page-head"><div><div class="eyebrow">Your social space</div><h1 class="page-title">Profile and settings</h1><p class="page-subtitle">Personalize your watch identity, connect with friends, and control your local TVNerd experience.</p></div><div class="head-actions"><button class="button" id="profile-share-button" type="button">↗ Share profile</button><button class="button primary" id="profile-save-button" type="button">Save profile</button></div></div><div class="profile-layout"><section class="profile-hero panel"><div class="profile-hero-art"></div><div class="profile-hero-content">${avatarMarkup(profile, 'profile-large-avatar')}<div class="profile-identity"><span class="eyebrow">LOCAL PROFILE</span><h2>${escapeHtml(profile.displayName)}</h2><span>${escapeHtml(profile.username || '@localviewer')}</span><p>${escapeHtml(profile.bio || 'No bio yet.')}</p></div></div><div class="profile-stat-row"><div><strong>${stats.watched}</strong><span>Episodes watched</span></div><div><strong>${stats.completed}</strong><span>Titles completed</span></div><div><strong>${stats.favorites}</strong><span>Favorites</span></div><div><strong>${profile.friends.length}</strong><span>Friends</span></div></div></section><section class="panel profile-editor"><div class="panel-header"><div><h2 class="panel-title">Personal details</h2><span class="section-meta">Visible on your local profile</span></div><span class="source-label">LOCAL ONLY</span></div><div class="profile-form"><label>Display name<input id="profile-name" value="${escapeHtml(profile.displayName)}" maxlength="40" placeholder="Your name"></label><label>Username<input id="profile-username" value="${escapeHtml(profile.username)}" maxlength="24" placeholder="@username"></label><label class="profile-bio-field">Bio<textarea id="profile-bio" maxlength="160" placeholder="Tell friends what you like to watch...">${escapeHtml(profile.bio)}</textarea></label><label>Profile picture<input id="profile-avatar-file" type="file" accept="image/png,image/jpeg,image/webp"></label><label>Activity visibility<select id="profile-visibility"><option value="private" ${profile.activityVisibility === 'private' ? 'selected' : ''}>Private</option><option value="friends" ${profile.activityVisibility === 'friends' ? 'selected' : ''}>Friends only</option><option value="public" ${profile.activityVisibility === 'public' ? 'selected' : ''}>Public preview</option></select></label></div></section><section class="panel badges-panel"><div class="panel-header"><div><h2 class="panel-title">Achievements</h2><span class="section-meta">${earnedBadges().length} of ${BADGES.length} unlocked</span></div><span class="source-label">GAMEPLAY</span></div>${badgeMarkup()}</section><section class="panel friends-panel"><div class="panel-header"><div><h2 class="panel-title">Friends</h2><span class="section-meta">Share what you are watching</span></div><button class="button" id="add-friend-button" type="button">+ Add friend</button></div><div class="friend-list">${profile.friends.map(friend => `<div class="friend-row"> <span class="friend-avatar">${escapeHtml(friend.initials)}</span><span class="friend-copy"><strong>${escapeHtml(friend.name)}</strong><small>${escapeHtml(friend.handle)} · ${escapeHtml(friend.status)}</small></span><button class="icon-action remove-friend" data-friend-id="${escapeHtml(friend.id)}" type="button" aria-label="Remove ${escapeHtml(friend.name)}">×</button></div>`).join('')}</div><div class="social-note">Friend accounts and cross-device sharing will activate when cloud accounts are connected. These contacts are stored locally for now.</div></section><section class="panel favorites-panel"><div class="panel-header"><div><h2 class="panel-title">Favorite titles</h2><span class="section-meta">Your personal must-watch shelf</span></div><span class="source-label">${favorites.length} saved</span></div>${favorites.length ? `<div class="cover-row">${favorites.map(show => coverCard(show, 'FAVORITE')).join('')}</div>` : '<div class="empty-state">Open a title and tap ☆ to build your favorites shelf.</div>'}</section><section class="panel sources-panel"><div class="panel-header"><div><h2 class="panel-title">Custom sources</h2><span class="section-meta">Your external stream &amp; torrent bookmarks</span></div><button class="button" id="add-source-button" type="button">+ Add source</button></div><div class="source-manage-list">${state.customSources.map(source => `<div class="source-manage-row"><span class="source-dot" style="background:${escapeHtml(source.color || '#e69b68')}"></span><span class="source-manage-copy"><strong>${escapeHtml(source.name)}</strong><small>${escapeHtml(source.home)}</small></span><button class="icon-action remove-source" data-source-id="${escapeHtml(source.id)}" type="button" aria-label="Remove ${escapeHtml(source.name)}">×</button></div>`).join('')}</div><form class="source-add-form" id="source-add-form" hidden><label>Name<input id="source-name" placeholder="e.g. Nyaa" maxlength="24"></label><label>Home URL<input id="source-home" placeholder="https://example.com/"></label><label>Search URL (use {q} for the query)<input id="source-search" placeholder="https://example.com/?q={q}"></label><button class="button primary" type="submit">Save source</button></form></section><section class="panel notif-panel"><div class="panel-header"><div><h2 class="panel-title">Notifications</h2><span class="section-meta">Choose what reaches your bell and when</span></div><span class="source-label">PREFERENCES</span></div><div class="setting-row"><div><strong>Release alerts</strong><span>Notify me when a tracked title is about to air.</span></div><button class="toggle ${state.notifPrefs.releases !== false ? 'on' : ''}" id="pref-releases" type="button" aria-label="Toggle release alerts"></button></div><div class="setting-row"><div><strong>Alert lead time</strong><span>How long before a release I get pinged.</span></div><select id="pref-lead"><option value="1" ${Number(state.notifPrefs.releaseLeadDays) === 1 ? 'selected' : ''}>1 day before</option><option value="3" ${Number(state.notifPrefs.releaseLeadDays) === 3 ? 'selected' : ''}>3 days before</option><option value="7" ${Number(state.notifPrefs.releaseLeadDays) === 7 ? 'selected' : ''}>7 days before</option></select></div><div class="setting-row"><div><strong>Suggestions</strong><span>Personalized “keep watching” nudges from your library.</span></div><button class="toggle ${state.notifPrefs.suggestions !== false ? 'on' : ''}" id="pref-suggestions" type="button" aria-label="Toggle suggestions"></button></div><div class="setting-row"><div><strong>Friend activity</strong><span>Updates when friends are watching something.</span></div><button class="toggle ${state.notifPrefs.friendActivity !== false ? 'on' : ''}" id="pref-friend" type="button" aria-label="Toggle friend activity"></button></div><div class="setting-row"><div><strong>Party reminders</strong><span>Countdown pings for your upcoming watch parties.</span></div><button class="toggle ${state.notifPrefs.partyReminders !== false ? 'on' : ''}" id="pref-party" type="button" aria-label="Toggle party reminders"></button></div><div class="setting-row"><div><strong>Weekly digest</strong><span>A Sunday summary of what you watched and what is next.</span></div><button class="toggle ${state.notifPrefs.weeklyDigest ? 'on' : ''}" id="pref-digest" type="button" aria-label="Toggle weekly digest"></button></div></section><section class="panel theme-panel"><div class="panel-header"><div><h2 class="panel-title">Theme</h2><span class="section-meta">Set the mood of your TVNerd</span></div><span class="source-label">APPEARANCE</span></div><div class="theme-grid">${THEMES.map(theme => `<button class="theme-option ${state.theme === theme.id ? 'active' : ''}" data-theme="${theme.id}" type="button" title="${theme.name}"><span class="theme-swatch" style="--swatch:${theme.swatch};--accent:${theme.accent}"></span><span>${theme.name}</span></button>`).join('')}</div></section><section class="panel settings-panel"><div class="panel-header"><div><h2 class="panel-title">App settings</h2><span class="section-meta">Local preferences and data</span></div></div><div class="setting-row"><div><strong>Release reminders</strong><span>Keep upcoming episodes on your radar.</span></div><button class="toggle ${state.reminders ? 'on' : ''}" id="settings-reminder" type="button" aria-label="Toggle reminders"></button></div><div class="setting-row"><div><strong>Google Calendar</strong><span>Add releases to Google Calendar through the event composer, or export the full guide as an .ics file.</span></div><div class="setting-actions"><button class="button" id="google-calendar-connect" type="button">${state.googleCalendarConnected ? 'Google connected' : 'Connect Google'}</button><button class="button" id="settings-calendar-export" type="button">Export .ics</button></div></div><div class="setting-row"><div><strong>Browser notifications</strong><span>Ask this device for permission to show release reminders.</span></div><button class="toggle ${state.browserNotifications ? 'on' : ''}" id="settings-notifications" type="button" aria-label="Toggle browser notifications"></button></div><div class="setting-row"><div><strong>Cloud sync</strong><span>Connect an account for desktop/mobile sharing.</span></div><button class="button primary" id="connect-account" type="button">Connect account</button></div><div class="setting-row"><div><strong>Export your data</strong><span>Download your library and history as a spreadsheet (CSV) or a full JSON backup.</span></div><div class="setting-actions"><button class="button" id="settings-export" type="button">Export JSON</button><button class="button" id="settings-export-csv" type="button">Export CSV</button></div></div><div class="setting-row"><div><strong>Session</strong><span>End this device session and return to the login screen.</span></div><button class="button" id="logout-button" type="button">Sign out</button></div><div class="setting-row"><div><strong>Legal &amp; policies</strong><span>Terms of Service, Privacy Policy, Cookie Policy, DMCA notice, and disclaimer.</span></div><button class="button" data-nav="legal" type="button">View policies</button></div></section></div>`;
  ...
  const bindNotifPref = (id, key) => { document.getElementById(id)?.addEventListener('click', () => { state.notifPrefs[key] = !(state.notifPrefs[key] !== false); saveState(); renderProfile(); generateNotifications(); updateBell(); showToast('Notification preference updated'); }); };
  bindNotifPref('pref-releases', 'releases'); bindNotifPref('pref-suggestions', 'suggestions'); bindNotifPref('pref-friend', 'friendActivity'); bindNotifPref('pref-party', 'partyReminders'); bindNotifPref('pref-digest', 'weeklyDigest');
  document.getElementById('pref-lead')?.addEventListener('change', event => { state.notifPrefs.releaseLeadDays = Number(event.target.value); saveState(); generateNotifications(); updateBell(); showToast('Release alert lead time updated'); });
  document.querySelectorAll('[data-theme]').forEach(button => button.addEventListener('click', () => { state.theme = button.dataset.theme; saveState(); applyTheme(); renderProfile(); showToast(`Theme set to ${THEMES.find(t => t.id === state.theme)?.name || state.theme}`); }));
  ...
  document.getElementById('add-source-button')?.addEventListener('click', () => { const form = document.getElementById('source-add-form'); if (form) form.hidden = !form.hidden; });
  document.getElementById('source-add-form')?.addEventListener('submit', event => { event.preventDefault(); const name = document.getElementById('source-name').value.trim(); const home = document.getElementById('source-home').value.trim(); const search = document.getElementById('source-search').value.trim(); if (!name || !home) { showToast('Name and home URL are required'); return; } state.customSources.push({ id: `src-${Date.now()}`, name: name.slice(0, 24), home, search: search || home, color: ['#e69b68', '#e3a5c8', '#8ec9c0', '#f1c84b'][state.customSources.length % 4] }); saveState(); renderProfile(); showToast(`${name} added to your sources`); });
  document.querySelectorAll('.remove-source').forEach(button => button.addEventListener('click', () => { const id = button.dataset.sourceId; const source = state.customSources.find(s => s.id === id); if (source?.default) { showToast('Default sources can be hidden but not removed — add your own instead'); return; } state.customSources = state.customSources.filter(s => s.id !== id); saveState(); renderProfile(); showToast('Source removed'); }));
...
function exportCsv() { const rows = [['Title', 'Type', 'Year', 'Genre', 'Status', 'Episodes watched', 'Total episodes', 'Progress %', 'Your rating', 'Note']]; catalog.filter(show => isUserAdded(show.id)).forEach(show => rows.push([show.title, show.type, show.year, (show.genre || '').replace(/,/g, ';'), show.status, watchedEpisodesFor(show), show.episodes, `${progressFor(show)}%`, state.ratings[show.id] || '', (state.notes[show.id] || '').replace(/[\n,]/g, ' ')])); const csv = rows.map(row => row.map(cell => `"${String(cell ?? '').replace(/"/g, '""')}"`).join(',')).join('\n'); const blob = new Blob([csv], { type: 'text/csv' }); const link = document.createElement('a'); link.href = URL.createObjectURL(blob); link.download = 'tvnerd-library.csv'; link.click(); URL.revokeObjectURL(link.href); showToast('CSV downloaded'); }
...
document.getElementById('notifications-button').addEventListener('click', event => { event.stopPropagation(); const panel = document.getElementById('notif-panel'); if (!panel) return; panel.hidden = !panel.hidden; if (!panel.hidden) { generateNotifications(); renderNotifPanel(); } });
document.addEventListener('click', event => { const panel = document.getElementById('notif-panel'); if (panel && !panel.hidden && !event.target.closest('.notif-wrap')) panel.hidden = true; });
document.getElementById('notif-clear')?.addEventListener('click', () => { state.notifications = []; saveState(); renderNotifPanel(); updateBell(); showToast('Notifications cleared'); });
document.getElementById('notif-mark-read')?.addEventListener('click', () => { state.notifications.forEach(n => { n.read = true; }); saveState(); renderNotifPanel(); updateBell(); showToast('All notifications marked as read'); });
document.addEventListener('click', event => { const item = event.target.closest('.notif-item'); if (!item) return; const showId = item.dataset.notifShow; const action = item.dataset.notifAction;  const changed = state.notifications.filter(n => n.showId === showId && !n.read); if (changed.length) { changed.forEach(n => { n.read = true; }); saveState(); renderNotifPanel(); updateBell(); }
  if (action === 'party') navigate('party');
  else if (showId) openShow(showId);
